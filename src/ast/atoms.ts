...
export const getOperationDefinition = atomFamily<gql.OperationDefinitionNode>(
  ...
  (path: string) => (get, set, node: gql.OperationDefinitionNode) => {
    if (!node) {
      set(getOperationDefinitionOperation(path), null),
        set(getOperationDefinitionName(path), null),
...
export const getVariableDefinition = atomFamily<gql.VariableDefinitionNode>(
  ...
  (path: string) => (get, set, node: gql.VariableDefinitionNode) => {
    if (!node) {
      set(getVariableDefinitionVariable(path), null),
...
export const getVariableDefinitions = atomFamily<gql.VariableDefinitionNode[]>(
  ...
  (path: string) => (get, set, nodes: gql.VariableDefinitionNode[]) => {
    if (!nodes) {
      get(getVariableDefinitionPaths(path)).map((path) =>
        set(getVariableDefinition(path), null)
      );
      ...
      for (var p in oldPaths) {
        if (!paths.includes(p)) {
          set(getVariableDefinition(p), null);
        }
      }

      set(getVariableDefinitionPaths(path), paths);
      nodes.forEach((node, index) => {
        set(getVariableDefinition(paths[index]), node);
      });
    }
  }
);
...
export const getField = atomFamily<gql.FieldNode>(
  ...
  (path: string) => (get, set, node: gql.FieldNode) => {
    if (!node) {
      set(getFieldAlias(path), null),
        set(getFieldName(path), null),
...
export const getFields = atomFamily<gql.FieldNode[]>(
  ...
  (path: string) => (get, set, nodes: gql.FieldNode[]) => {
    if (!nodes) {
      get(getFieldPaths(path)).map((path) => set(getField(path), null));
      ...
      set(getFieldPaths(path), paths);
      nodes.forEach((node, index) => {
        set(getField(paths[index]), node);
      });
    }
  }
);
...
export const getArguments = atomFamily<gql.ArgumentNode[]>(
  ...
  (path: string) => (get, set, nodes: gql.ArgumentNode[]) => {
    if (!nodes) {
      get(getArgumentPaths(path)).map((path) => set(getArgument(path), null));
      ...
      for (var p in oldPaths) {
        if (!paths.includes(p)) {
          set(getArgument(p), null);
        }
      }

      set(getArgumentPaths(path), paths);
      nodes.forEach((node, index) => {
        set(getArgument(paths[index]), node);
      });
    }
  }
);
...
export const getFragmentDefinition = atomFamily<gql.FragmentDefinitionNode>(
  ...
  (path: string) => (get, set, node: gql.FragmentDefinitionNode) => {
    if (!node) {
      set(getFragmentDefinitionName(path), null),
        set(getFragmentDefinitionVariableDefinitions(path), null),
...
export const getObjectFields = atomFamily<gql.ObjectFieldNode[]>(
  ...
  (path: string) => (get, set, nodes: gql.ObjectFieldNode[]) => {
    if (!nodes) {
      get(getObjectFieldPaths(path)).map((path) =>
        set(getObjectField(path), null)
      );
      ...
      for (var p in oldPaths) {
        if (!paths.includes(p)) {
          set(getObjectField(p), null);
        }
      }

      set(getObjectFieldPaths(path), paths);
      nodes.forEach((node, index) => {
        set(getObjectField(paths[index]), node);
      });
    }
  }
);
...
export const getDirectives = atomFamily<gql.DirectiveNode[]>(
  ...
  (path: string) => (get, set, nodes: gql.DirectiveNode[]) => {
    if (!nodes) {
      get(getDirectivePaths(path)).map((path) => set(getDirective(path), null));
      ...
      for (var p in oldPaths) {
        if (!paths.includes(p)) {
          set(getDirective(p), null);
        }
      }

      set(getDirectivePaths(path), paths);
      nodes.forEach((node, index) => {
        set(getDirective(paths[index]), node);
      });
    }
  }
);
  ...
  (path: string) => (get, set, node: gql.ASTNode) => {
    if (!node) {
      ...
      switch (kind) {
        ...
        case "Document": {
          return set(getDocument(path), null);
        }
        case "OperationDefinition": {
          return set(getOperationDefinition(path), null);
        }
        case "VariableDefinition": {
          return set(getVariableDefinition(path), null);
        }
        case "Variable": {
          return set(getVariable(path), null);
        }
        case "SelectionSet": {
          return set(getSelectionSet(path), null);
        }
        case "Field": {
          return set(getField(path), null);
        }
        case "Argument": {
          return set(getArgument(path), null);
        }
        case "FragmentSpread": {
          return set(getFragmentSpread(path), null);
        }
        case "InlineFragment": {
          return set(getInlineFragment(path), null);
        }
        case "FragmentDefinition": {
          return set(getFragmentDefinition(path), null);
        }
        case "IntValue": {
          return set(getIntValue(path), null);
        }
        case "FloatValue": {
          return set(getFloatValue(path), null);
        }
        case "StringValue": {
          return set(getStringValue(path), null);
        }
        case "BooleanValue": {
          return set(getBooleanValue(path), null);
        }
        case "NullValue": {
          return set(getNullValue(path), null);
        }
        case "EnumValue": {
          return set(getEnumValue(path), null);
        }
        case "ListValue": {
          return set(getListValue(path), null);
        }
        case "ObjectValue": {
          return set(getObjectValue(path), null);
        }
        case "ObjectField": {
          return set(getObjectField(path), null);
        }
        case "Directive": {
          return set(getDirective(path), null);
        }
        case "NamedType": {
          return set(getNamedType(path), null);
        }
        case "ListType": {
          return set(getListType(path), null);
        }
        case "NonNullType": {
          return set(getNonNullType(path), null);
        }
      }
      return;
    } else {
...
export const getDefinitions = atomFamily<gql.DefinitionNode[]>(
  ...
  (path: string) => (get, set, nodes: gql.DefinitionNode[]) => {
    if (!nodes) {
      get(getDefinitionPaths(path)).map((path) =>
        set(getDefinition(path), null)
      );
      ...
      for (var p in oldPaths) {
        if (!paths.includes(p)) {
          set(getDefinition(p), null);
        }
      }

      set(getDefinitionPaths(path), paths);
      nodes.forEach((node, index) => {
        set(getDefinition(paths[index]), node);
      });
    }
  }
);
...
export const getSelections = atomFamily<gql.SelectionNode[]>(
  ...
  (path: string) => (get, set, nodes: gql.SelectionNode[]) => {
    if (!nodes) {
      get(getSelectionPaths(path)).map((path) => set(getSelection(path), null));
      ...
      for (var p in oldPaths) {
        if (!paths.includes(p)) {
          set(getSelection(p), null);
        }
      }

      set(getSelectionPaths(path), paths);
      nodes.forEach((node, index) => {
        set(getSelection(paths[index]), node);
      });
    }
  }
);
...
export const getValues = atomFamily<gql.ValueNode[]>(
  ...
  (path: string) => (get, set, nodes: gql.ValueNode[]) => {
    if (!nodes) {
      get(getValuePaths(path)).map((path) => set(getValue(path), null));
      ...
      set(getValuePaths(path), paths);
      nodes.forEach((node, index) => {
        set(getValue(paths[index]), node);
      });
    }
  }
);
