...
initialize(
    ...
    class MonacoGraphQLWorker extends BaseWorker {
        ...
        doValidation(uri) {
            return this.worker.doValidation(uri)
        }
        ...
        provideDocumentFormattingEdits: BaseWorker['provideDocumentFormattingEdits'] = async (model) => {
            ...
            return [{ range: { startLineNumber: 1, endLineNumber: lines.length, startColumn: 0, endColumn: lines[lines.length - 1].length }, text: info }];
        }
        provideCompletionItems: BaseWorker['provideCompletionItems'] = async (model, pos, ctx) => {
            const info = await this.worker.doComplete(model.uri.path, pos);
            return {
                suggestions: info as any
            }
        }
        resolveCompletionItem: BaseWorker['resolveCompletionItem'] = async (model, pos, item) => {
            return item;
        }
    }
);